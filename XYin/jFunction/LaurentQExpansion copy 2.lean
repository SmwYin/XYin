...
lemma LaurentCoeff_exists_greatest
  (hfcusp : MeromorphicAt (cuspFunction h f) (0 : ℂ)) :
    ∃ M0 : ℤ, M0 ≤ 0 ∧ (∀ n : ℤ, n < M0 → ∃ R : ℝ, 0 < R ∧
    (2 * π * I)⁻¹ * ∮ z in C(0, R / 2), cuspFunction h f z / z ^ (n + 1) = 0) ∧
    ∀ M : ℤ, M ≤ 0 → (∀ n : ℤ, n < M → ∃ R : ℝ, 0 < R ∧
    (2 * π * I)⁻¹ * ∮ z in C(0, R / 2), cuspFunction h f z / z ^ (n + 1) = 0) → M ≤ M0 := by
  let P : ℤ → Prop := fun M => M ≤ 0 ∧ ∀ n : ℤ, n < M → ∃ R : ℝ, 0 < R ∧
    (2 * π * I)⁻¹ * ∮ z in C(0, R / 2), cuspFunction h f z / z ^ (n + 1) = 0
  have hbdd : ∃ b : ℤ, ∀ z : ℤ, P z → z ≤ b := by
    refine ⟨0, ?_⟩
    intro z hz
    exact hz.1
  have hinh : ∃ z : ℤ, P z := by
    simpa [P] using (LaurentCoeff_pre hfcusp)
  rcases Int.exists_greatest_of_bdd hbdd hinh with ⟨M0, hM0, hmax⟩
  refine ⟨M0, hM0.1, hM0.2, ?_⟩
  intro M hM hprop
  exact hmax M ⟨hM, hprop⟩

lemma Laurent_pre {h : ℝ} {f : ℍ → ℂ} (hh : 0 < h)
    (hfcusp : MeromorphicAt (cuspFunction h f) (0 : ℂ)) :
    ∃ (M : ℕ), DifferentiableAt ℂ (cuspFunction h (fun (τ : ℍ) ↦ (𝕢 h τ) ^ M * f τ)) 0 := by
  rcases hfcusp with ⟨M, hM⟩
  refine ⟨M, ?_⟩
  ...
  have hEq_nonzero : ∀ {q : ℂ}, q ≠ 0 → ‖q‖ < 1 → F q = G q := by
    ...
    calc
      ...
      _ = q ^ M * f (UpperHalfPlane.ofComplex (Periodic.invQParam h q)) := by simp [hqparam]
      _ = q ^ M * cuspFunction h f q := by simp [hright]
      _ = G q := by simp [G]
...
lemma Laurent_pre' {h : ℝ} {f : ℍ → ℂ} (hh : 0 < h)
    (hfcusp : MeromorphicAt (cuspFunction h f) (0 : ℂ)) :
    ∃ M : ℕ, DifferentiableAt ℂ (cuspFunction h (fun (τ : ℍ) ↦ (𝕢 h τ) ^ M * f τ)) 0 ∧
    ∀ m : ℕ, DifferentiableAt ℂ (cuspFunction h (fun (τ : ℍ) ↦ (𝕢 h τ) ^ m * f τ)) 0 → M ≤ m := by
  classical
  let P : ℕ → Prop := fun M =>
    DifferentiableAt ℂ (cuspFunction h (fun (τ : ℍ) ↦ (𝕢 h τ) ^ M * f τ)) 0
  have hEx : ∃ M : ℕ, P M := by
    simpa [P] using (Laurent_pre (h := h) (f := f) hh hfcusp)
  refine ⟨Nat.find hEx, Nat.find_spec hEx, ?_⟩
  intro m hm
  exact Nat.find_min' hEx hm

/-
lemma Laurent_pre2 {h : ℝ} {f : ℍ → ℂ} (hh : 0 < h)
    (hfcusp : MeromorphicAt (cuspFunction h f) (0 : ℂ)) :
    ∃ (M : ℕ), DifferentiableAt ℂ (cuspFunction h (fun (τ : ℍ) ↦ (𝕢 h τ) ^ M * f τ)) 0
    ∧ (qExpansion h (fun (τ : ℍ) ↦ (𝕢 h τ) ^ M * f τ)).coeff 0 ≠ 0 := by
  sorry
-/

def nonnegOrder' {h : ℝ} {f : ℍ → ℂ} (hh : 0 < h)
    (hfcusp : MeromorphicAt (cuspFunction h f) (0 : ℂ)) :=
  (Laurent_pre' hh hfcusp).choose

lemma find_nonnegOrder' {n : ℕ} {h : ℝ} {f : ℍ → ℂ} (hn : n ≠ 0) (hh : 0 < h)
    (hfcusp : MeromorphicAt (cuspFunction h f) (0 : ℂ))
    (hf : (qExpansion h (fun (τ : ℍ) ↦ (𝕢 h τ) ^ n * f τ)).coeff 0 ≠ 0) :
    nonnegOrder' hh hfcusp = n := by
  sorry

def laurentqExpansion2 (h : ℝ) (f : ℍ → ℂ) (hh : 0 < h)
    (hf : MeromorphicAt (cuspFunction h f) (0 : ℂ)) :
    LaurentSeries ℂ :=
  HahnSeries.single (-(nonnegOrder' hh hf : ℤ)) (1 : ℂ) *
    (HahnSeries.ofPowerSeries ℤ ℂ) (qExpansion h (fun τ ↦ (𝕢 h τ) ^ (nonnegOrder' hh hf) * f τ))

def nonnegOrder {h : ℝ} {f : ℍ → ℂ} (hfcusp : MeromorphicAt (cuspFunction h f) (0 : ℂ)) : ℤ :=
  (LaurentCoeff_exists_greatest hfcusp).choose

def laurentqExpansion' (h : ℝ) (f : ℍ → ℂ) (hf : MeromorphicAt (cuspFunction h f) (0 : ℂ)) :
    LaurentSeries ℂ :=
  HahnSeries.single (nonnegOrder hf) (1 : ℂ) *
    (HahnSeries.ofPowerSeries ℤ ℂ) (qExpansion h (fun τ ↦ (𝕢 h τ) ^ (-nonnegOrder hf) * f τ))
...
noncomputable def laurentCoeffRadius (h : ℝ) (f : ℍ → ℂ)
  ...
  let R0 : ℝ := Classical.choose
    ((Classical.choose_spec (LaurentCoeff_pre hfcusp)).2 (M - 1) (by
      have hlt : M - 1 < M := sub_lt_self M zero_lt_one
      simp [M, laurentCoeffCutoff]))
  min R0 1
...
lemma laurentCoeffRadius_pos (hfcusp : MeromorphicAt (cuspFunction h f) (0 : ℂ)) :
  ...
  have hlt :
      laurentCoeffCutoff h f hfcusp - 1 < laurentCoeffCutoff h f hfcusp := by
    exact sub_lt_self _ zero_lt_one
  exact
    (Classical.choose_spec
      ((Classical.choose_spec (LaurentCoeff_pre (h := h) (f := f) hfcusp)).2
        (laurentCoeffCutoff h f hfcusp - 1) hlt)).1
...
theorem Laurent_to_qExpansion'
  ...
  have hRhalf_lt1 : R / 2 < 1 := by
    linarith
  have hcut_le0 : laurentCoeffCutoff h f hfcusp ≤ 0 :=
