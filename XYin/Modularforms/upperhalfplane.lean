...
lemma pnat_div_upper (n : ℕ+) (z : ℍ) : 0 < (-(n : ℂ) / z).im := by
  ...
  simp only [Int.cast_neg, Int.cast_natCast, neg_im, natCast_im, neg_zero, coe_re, zero_mul,
    zero_div, neg_re, natCast_re, coe_im, neg_mul, zero_sub, Left.neg_pos_iff]
  rw [@div_neg_iff]
  right
...
lemma pos_nat_div_upper (n : ℤ) (hn : 0 < n) (z : ℍ) : 0 < (-(n : ℂ) / z).im := by
  ...
  norm_cast
  rw [div_im]
  simp [Int.cast_neg, neg_im, neg_zero, coe_re, zero_mul, zero_div, neg_re, coe_im, neg_mul,
    zero_sub, Left.neg_pos_iff]
  rw [div_neg_iff]
  right
  simp [Left.neg_neg_iff, Complex.normSq_pos, ne_eq]
