...
lemma E4_q_exp : (fun m => (qExpansion 1 E₄).coeff m) =
  ...
  next h =>
    ...
    left
    simp [ bernoulli, bernoulli'_four, Rat.cast_mul, Rat.cast_pow, Rat.cast_neg,
    Rat.cast_one, Rat.cast_div, Rat.cast_ofNat, Nat.factorial,
...
theorem E₂_mul_E₄_sub_E₆ (z : ℍ) :
    (E₂ z) * (E₄ z) - (E₆ z) = 720 * ∑' (n : ℕ+), n * (sigma 3 n) * cexp (2 * π * Complex.I * n * z) := by
  sorry
